...
export class BookofferformComponent {
 ...
 getClinicSchedualByClinicDayId(day: any) {
  ...
  const DayId = day.id;
  const MedicalExaminationTypeId = this.selectedAppointment;
  ...
  this.service
    ...
    .subscribe(res => {
      ...
      if (res?.Data?.length > 0) {
        ...
        const booked = res.Data[0].BookedAppointments || [];

        booked.forEach((a: any) => {
          const time = a.split(' ')[1];
      ...
      if (this.times.length > 0) {
        this.selectedDayId = this.times[0]['SchedualId'];
      }
