...
export class AppService {
  ...
  constructor(
    ...
    private langserv: LangServService
  ) { }
  ...
  GetDoctorPolyClinic(ClinicId: any) {
    return this.http.get(
      `${environment.apiUrl}/Doctor/GetDoctorByClinicId?ClinicId=${ClinicId}&IsApproved=${true}`
    );
  }
  GetAllQandAByPatientId( lang: string = 'en') {
  return this.http.get(
    `${environment.apiUrl}/Patient/GetAllQandAByPatientId?lang=${lang}`
  );
}
  getDoctorDetail(
    doctorId: any,
  ...
  getTopSpecialist() {
    return this.http.get<any>(
      `${environment.apiUrl}/Specialist/GetTop10Specialist`
    );
  }
  ...
  getBookingForAnotherPatient(){
    return this.http.get<any>(
      `${environment.apiUrl}/PatientRelationshipType/GetAllPatientRelationshipType`
    );
  }
  getFoodAllergy() {
    return this.http.get<any>(
  ...
  getUserNotifi() {
    return this.http.get<any>(
      `${environment.apiUrl}/Notification/GetUserNotifications`
    );
  }
  ...
  setDefaultMetaTags() {
    ...
    const lang = this.langserv.getLang();
