...
import { SearchFormComponent, TableEnum } from '../../shared/search-form/search-form.component';
...
export class FindADoctorBySubSpecialtyComponent {
  ...
  constructor(
    ...
    this.StorageService.setItem('PAGE_CURRENT_KEY','find-a-doctor-by-sub-specialty')
  ...
  ngOnInit(): void {
    ...
    this.StorageService.setItem('currentUrl', decodeURIComponent(this.router.url));
    // if(!this.currentUrl){
    // }
    if(!this.alternativeUrl){
      let alternativeUrl = this.routesPipe.transform('find-a-doctor-by-sub-specialty', [(this.translocoService.getActiveLang() == 'en' ? 'ar' : 'en')]);
      this.StorageService.setItem('alternativeUrl', decodeURIComponent(alternativeUrl));

    }

    // this.storageService.setItem('alternativeUrl', decodeURIComponent(currentUrl));
    // this.storageService.setItem('currentUrl', decodeURIComponent(alternativeUrl));
    ...
    // update meta title and meta description for doctor search
  ...
  // this.StorageService.setItem('FeesFrom',"0");
  ...
  setTitle(){
    ...
    //     let subSpecialist = this.replaceDashWithSpace(this.params['subSpecialist'])
  ...
  getSubSpecialist(id:any){
    return this.service.getListOfSubSpecialistList(id).pipe(map(res=>res['Data'])).subscribe(res=>{
  ...
  if (this.subSpeciality) {
    ...
    .getNameInOtherLanguage(TableEnum.SubSpeciality, this.replaceDashWithSpace(this.subSpeciality))
    ...
    if (this.speciality) {
      ...
      // all specialities
      console.log(this.speciality)
      if(this.speciality == 'all-specialities' || this.speciality == 'كل-التخصصات'){
        requests.push(of({}));
          return
      }
      const specialityReq = this.service
        .getNameInOtherLanguage(TableEnum.Speciality, this.replaceDashWithSpace(this.speciality))
        .pipe(map((res: any) => ({
          SpecialistId: res.Id,
          res : res
    ...
    if (this.city) {
      ...
      const cityReq = this.service
        .getNameInOtherLanguage(TableEnum.City, this.replaceDashWithSpace(this.city))
    ...
    if (this.area) {
      const areaReq = this.service
        .getNameInOtherLanguage(TableEnum.Area, this.replaceDashWithSpace(this.area))
    ...
    if(this.params['service']){
      const serviceReq = this.service
        .getNameInOtherLanguage(TableEnum.MediacalType, this.replaceDashWithSpace(this.params['service']))
        .pipe(map((res: any) => ({ MedicalExaminationTypeId: res.Id , res : res})));
    ...
    forkJoin(requests).subscribe({
      ...
      next: (results: any[]) => {
        ...
        // console.log(res)
        //console.log(alternativeUrl)
        this.StorageService.setItem('alternativeUrl', this.replaceSpaceWithDash(decodeURIComponent(alternativeUrl)));
        ...
        if (this.params['price']) {
          const [FeesFrom, FeesTo] = this.params['price'].split(',').map((value:any) => value !== 'null' ? Number(value) : null);
        ...
        // this.spinner.show()
        this.setPageTitle()
        this.service.findDoctorsByFilters(object)
  ...
  setPageTitle(doctorsCount:any = null){
    ...
    this.metadataService.updateMetaDescription(title,'find-a-doctor-by-sub-specialty')


    this.StorageService.setItem('filterTitle',JSON.stringify({
      ar:titleAr,
      en:titleEn
    }),'session')
  }
  ...
  filter(MaxResultCount = 10, SkipCount = 0,specialistId?:any,cityId?:any,areaId?:any) {
//     this.loading = true;
...
//     })
  ...
  setParam(key:any, value:any){
    ...
    this.router.navigate([this.routesPipe.transform('find-a-doctor-by-sub-specialty')],
     { queryParams: params })
  }
  ...
  bookFor(event:any,doctor:any){
    ...
    this.router.navigate([this.routesPipe.transform('doctor-profile'),doctor.DoctorId , doctor.DoctorName]
  ...
  goToSubSpecial(name:any, id:any,SpecialistName:any) {
    ...
    this.router.navigate([this.routesPipe.transform('find-a-doctor-by-sub-specialty') ,this.replaceSpaceWithDash(name)]).then(res=>{
      this.doctors = []
    ...
    getSubSpecialtyLink(subspecialist:any,doctor:any){
      ...
      let url = this.routesPipe.transform('find-a-doctor-by-sub-specialty') + '/' + this.replaceSpaceWithDash(subspecialist.Name)
      if(this.speciality){
  ...
  goToSpecialist(event:any,Id:any,Name:any){
    ...
    // /doctors/
    this.router.navigate([this.routesPipe.transform('find-a-doctor'),this.replaceSpaceWithDash(Name)])
  }
