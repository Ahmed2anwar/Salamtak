...
export class FindADoctorComponent {
  ...
  ngOnInit(): void {
    ...
    if (!this.alternativeUrl) {
      let alternativeUrl = this.routesPipe.transform('find-a-doctor', [(this.translocoService.getActiveLang() == 'en' ? 'ar' : 'en')]);
  ...
  setFilter($event: any, key: any, id: any, name: any) {
    ...
    // Remove leading/trailing spaces & replace spaces with dash
    name = name.trim();
    ...
    if (!queryParamsKeys.includes(key)) {
      if (key === 'speciality') {
        this.router.navigate([this.routesPipe.transform('find-a-doctor') + '/' + name], { queryParams: this.params })
          .then(() => {
  ...
  doctorsSearch() {
    ...
    if (isPlatformBrowser(this.platformId)) {
      searchForm = JSON.parse(localStorage.getItem('search-form') || '{}');
    }
